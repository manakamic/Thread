...
// C++20 時代のスレッド
// シンプル処理
// std::jthread + std::atomic<bool> を使ったスレッドの完了待ちを行うサンプル
...
bool CheckThreadFinished(const std::vector<std::atomic<bool>>& threadFinished);
...
int CALLBACK WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow) {
    ...
    std::vector<std::atomic<bool>> threadFinished(threadNum); // 完了フラグ
    ...
    while (ProcessMessage() != -1) {
        ...
        if (!allThreadFinised && startThreads) {
            allThreadFinised = CheckThreadFinished(threadFinished);
...
void MandelbrotThreadTask(std::stop_token stopToken, const int startY, const int endY, std::atomic<bool>& finished, std::vector<unsigned int>& pixelArray) {
    ...
    if (!stopToken.stop_requested()) {
        // finished = true; と書けるが、明示的な atomic 操作を示すため store メソッドを使用
        finished.store(true);
    }
}
...
bool CreateThread(const int threadNum,
    ...
    for (auto i = 0; i < threadNum; ++i) {
        ...
        // スレッドに渡す際には参照型でもコピーが作られてしまうので std::ref で参照オブジェクトにする必要がある
